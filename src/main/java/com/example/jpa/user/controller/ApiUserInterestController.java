...
public class ApiUserInterestController {
    ...
    public ResponseEntity<?> interestUser(@PathVariable Long id, @RequestHeader("Z-TOKEN") String token) {
        String email = "";
        try {
    ...
    public ResponseEntity<?> deleteInterestUser(@PathVariable Long id, @RequestHeader("Z-TOKEN") String token) {
        String email = "";
        try {
            email = JWTUtils.getIssuer(token);
        } catch (JWTVerificationException e) {
