...
public class ApiBoardBookmarkController {
    ...
    public ResponseEntity<?> boardBookmark(@PathVariable Long id, @RequestHeader("Z-TOKEN") String token) {
        String email = "";
        try {
    ...
    public ResponseEntity<?> deleteBookmark(@PathVariable Long id, @RequestHeader("Z-TOKEN") String token) {
        String email = "";
        try {
            email = JWTUtils.getIssuer(token);
        } catch (JWTVerificationException e) {
