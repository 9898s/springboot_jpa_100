...
public class ApiBoardScrapController {
    ...
    public ResponseEntity<?> boardScrap(@PathVariable Long id, @RequestHeader("Z-TOKEN") String token) {
        String email = "";
        try {
    ...
    public ResponseEntity<?> deleteBoardScrap(@PathVariable Long id, @RequestHeader("Z-TOKEN") String token) {
        String email = "";
        try {
            email = JWTUtils.getIssuer(token);
        } catch (JWTVerificationException e) {
